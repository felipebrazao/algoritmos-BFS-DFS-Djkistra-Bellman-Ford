class GrafoBellmanFord:
    ...
    def bellman_ford(self, origem):
        ...
        for iteracao in range(self.V - 1):
            atualizacoes = 0
            
            for origem_aresta, destino_aresta, peso in self.arestas:
                if distancia[origem_aresta] != float('inf') and \
                   distancia[origem_aresta] + peso < distancia[destino_aresta]:
                    
                    distancia[destino_aresta] = distancia[origem_aresta] + peso
                    predecessor[destino_aresta] = origem_aresta
                    atualizacoes += 1
        ...
        return distancia, predecessor, ciclo_negativo
...
def exemplo_sistema_transporte():
    ...
    print("\nALGORITMO DE BELLMAN-FORD - Sistema de Transporte")
    
    for origem, destino, peso in rotas:
        grafo.adicionar_aresta(origem, destino, peso)
    
    origem = 0
    distancia, predecessor, ciclo_negativo = grafo.bellman_ford(origem)
    ...
    print("\n" + "="*70)
    origem2 = 5
    distancia2, predecessor2, ciclo_negativo2 = grafo.bellman_ford(origem2)
